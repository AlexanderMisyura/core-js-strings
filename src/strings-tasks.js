...
function removeLastOccurrences(str, value) {
  const occurrenceInd = str.lastIndexOf(value);
  if (occurrenceInd === -1) return str;
  return str
    .slice(0, occurrenceInd)
    .concat(str.slice(occurrenceInd + value.length));
...
function countVowels(str) {
  ...
  let vowelCounter = 0;
  [...str].forEach((letter) => {
    if (vowels.includes(letter)) vowelCounter += 1;
  });
  return vowelCounter;
}
...
function isPalindrome(str) {
  const editedStr = str.toLowerCase().replace(/[` ,.;:!?`]/g, '');
  for (let i = 0; i <= editedStr.length / 2; i += 1) {
    if (editedStr[i] !== editedStr[editedStr.length - 1 - i]) return false;
  }
  return true;
}
...
function extractNameFromTemplate(value) {
  const nameLength = value.length - 8;
  return [...value].splice(7, nameLength).join('');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/./g, (char) => {
    if (alphabet.includes(char.toLowerCase())) {
      ...
      const cypheredInd = (ind + 13) % 26;

      return char.toUpperCase() === char
        ? alphabet[cypheredInd].toUpperCase()
        : alphabet[cypheredInd];
    }
    return char;
  });
}
